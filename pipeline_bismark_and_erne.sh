...
echo "call methylation with the bismark caller, generating methylation annotations in cov format"
/usr/bin/time -v bismark_methylation_extractor -p --output $wd --gzip --CX --bedGraph --genome_folder $genome_folder $samfile 2> $bismark_caller_benchmark

echo "remove all files generated by bismark that are not of interest"
